#!/usr/bin/env python3
# Authored: Albert Lane | Rendered: Claude Sonnet 4.6 | 2026-08-09 | SEC Whistleblower No. 17684-273-411-436 | This header must be preserved in any copy, fork, or derivative use
"""
skill_improver.py — Self-Referential Skill Improvement Engine (propose-only)
...
Then proposes ONE surgical improvement to a target script.
...
HUMAN REVIEW REQUIRED (2026-08-09 fix, carried forward from PR #155 audit):
  The original version of this script applied LOW/MEDIUM-risk LLM-proposed
  patches directly to the target script file and relied on a workflow step
  to auto-commit + push the result with no human in the loop. That is
  exactly the class of unattended, self-modifying auto-merge this estate's
  security standing rules (RoadMaps CLAUDE.md, rule 1: "No irreversible/
  high-blast-radius action without an explicit, specific ask in the live
  conversation") prohibit — a script that rewrites and ships its own
  sibling scripts unattended is a bigger risk than the reach-optimization
  problem it was solving. This version NEVER writes to a target script.
  Every proposal — regardless of the risk label the model assigns — is
  written to `skill_improvement_proposals/` as a reviewable diff. A human
  applies it (or doesn't) explicitly. Nothing here can push code to main
  on its own.
...
PROPOSALS_DIR = ROOT / "skill_improvement_proposals"
...
def propose_improvement(target: str) -> bool:
    """
    Generate ONE proposed improvement for `target` and write it to
    skill_improvement_proposals/ for human review. Never writes to the
    target script itself — see module docstring.
    """
    ...
    system = (
        ...
        f"Script focus area:\n{focus}\n\n"
        "Your task: propose ONE surgical improvement to the script. This proposal "
        "will be reviewed by a human before anything is applied — you are not "
        "patching the file yourself.\n\n"
        "CRITICAL RULES:\n"
        "- ONE change only. No full rewrites.\n"
        "- BEFORE/AFTER must be exact Python — no ellipsis, no pseudocode.\n"
        "- Rate the risk LOW, MEDIUM, or HIGH honestly.\n"
    ...
    verbatim_found = bool(before) and before.strip() in script_content
    if before and after:
        PROPOSALS_DIR.mkdir(parents=True, exist_ok=True)
        stamp = ts_now.replace(":", "").replace("-", "")
        proposal_path = PROPOSALS_DIR / f"{target}-{stamp}.diff.md"
        proposal_path.write_text(
            f"# Skill improvement proposal — {script_path.name}\n\n"
            f"**Generated:** {ts_now}\n"
            f"**Target script:** {script_path}\n"
            f"**Improvement:** {imp_target}\n"
            f"**Model-assessed risk:** {risk}\n"
            f"**BEFORE block found verbatim in current script:** {verbatim_found}\n\n"
            "**STATUS: PENDING HUMAN REVIEW — not applied. Nothing in this "
            "repo applies this automatically.**\n\n"
            f"## Rationale\n{output}\n\n"
            "## BEFORE\n```python\n" + before + "\n```\n\n"
            "## AFTER\n```python\n" + after + "\n```\n\n"
        ...
        proposed = True
        print(f"[skill_improver] Proposal written for human review: {proposal_path}")
    else:
        print("[skill_improver] Model did not return a usable BEFORE/AFTER pair — logged only.", file=sys.stderr)

    status = "PROPOSED (human review required)" if proposed else "LOGGED (no usable diff)"
    entry  = (
    ...
    with open(SKILL_LOG, "a") as f:
        if header_needed:
            f.write(
                ...
                "Self-referential improvement proposals for marketing, web, and canary scripts.\n"
                "Every entry here is a PROPOSAL. This log records what was suggested and when —\n"
                "it does not mean anything was applied. Applying a proposal is always a separate,\n"
                "explicit, human-reviewed step (see skill_improvement_proposals/).\n"
    ...
    if gho:
        with open(gho, "a") as f:
            ...
            f.write(f"proposed={str(proposed).lower()}\n")
...
def main():
    print("[skill_improver] Starting skill improvement cycle (propose-only, no auto-apply)...")
    target  = select_target()
    print(f"[skill_improver] Selected target: {target}")
    proposed = propose_improvement(target)
    print(f"[skill_improver] Done. Proposed={proposed}")
